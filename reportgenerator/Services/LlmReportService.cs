...
public class LlmReportService
{
    ...
    public async Task<LlmGenerateResponse> GenerateAsync(LlmGenerateRequest request, CancellationToken ct)
    {
        ...
        var content = await _openRouter.ChatAsync(systemPrompt, userPrompt, request.Model, ct);
    ...
    {
        return "You generate report definitions for a report designer. Return ONLY valid JSON (no markdown, no code fences). The JSON must have: reportObjects (array), canvasSettings (object), parameters (array or null), metadata (object or null). metadata fields: name, description, author. Each reportObjects item must include: id (string), type (one of: text,image,chart,table,datatable,barcode,dataRegion,header,footer,line,rectangle,ellipse,polygon,polyline), x,y,width,height (numbers), properties (object). IMPORTANT: The frontend expects object type strings to be lower-case/camelCase like: text, image, chart, table, datatable, barcode, dataRegion, header, footer, line, rectangle, ellipse, polygon, polyline (do NOT use PascalCase like 'DataRegion' or 'Table'). Use header/footer as layout containers that repeat on every page: create at most one header and one footer; then place text objects inside their area (coordinates within the header/footer rectangle) for title/description/author as needed. Prefer the datatable object over table when showing tabular data; datatable supports grouping (properties.dataTableGroupBy), optional totals (properties.dataTableTotalsRow), and per-column widths/styles (properties.columnWidths, properties.tableHeaderStyle, properties.tableHeaderCellStyles). For data-driven reports, include at least one dataRegion with properties.dataSource. If you need filtering, joins, grouping, ordering, or computed columns, use a SQLRest query data source: {\"type\":\"sqlrest\",\"sourceType\":\"query\",\"sql\":\"SELECT ...\"}. Otherwise use a simple table data source: {\"type\":\"sqlrest\",\"sourceType\":\"table\",\"name\":\"schema.table\",\"tableName\":\"schema.table\"}. Place at least one datatable or table object inside that region and set its properties.columns (array of column names). Use A4 canvasSettings matching editor defaults: width 794, height 1123, page preset 'A4', orientation 'portrait', margins {top:40,right:40,bottom:40,left:40}, showGrid true, snapToGrid true, gridSize 20, zoom 1.";
    }
