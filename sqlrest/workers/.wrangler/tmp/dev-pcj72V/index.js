...
// .wrangler/tmp/bundle-hNa5cW/checked-fetch.js
...
// .wrangler/tmp/bundle-hNa5cW/strip-cf-connecting-ip-header.js
...
var DatabaseService = class {
  ...
  async getAllSchemasAndTables(env) {
    try {
      ...
      if (env.DB) {
        ...
        `).all();
        const schemasAndTables = /* @__PURE__ */ new Map();
  ...
  async getTableColumns(env, schema, table) {
    try {
      await this.connect(env);
      if (env.DB) {
        const result = await env.DB.prepare(`
  ...
  async executeQuery(env, schema, table, operation, params = {}) {
    try {
      await this.connect(env);
  ...
  async executeSelectQuery(env, schema, table, params) {
    const page = parseInt(params.page) || 1;
    const pageSize = Math.min(parseInt(params.pageSize) || 100, 1e3);
    ...
    const dataResult = await env.DB.prepare(dataQuery).bind(...queryParams, pageSize, offset).all();
  ...
  async executeInsertQuery(env, schema, table, params) {
    const columns = Object.keys(params).filter((key) => params[key] !== void 0);
    const values = columns.map((col) => params[col]);
    const placeholders = columns.map(() => "?").join(", ");
    const query = `INSERT INTO ${table} (${columns.join(", ")}) VALUES (${placeholders})`;
    const result = await env.DB.prepare(query).bind(...values).run();
    if (result.success && result.meta.last_row_id) {
      const insertedRecord = await env.DB.prepare(`SELECT * FROM ${table} WHERE rowid = ?`).bind(result.meta.last_row_id).first();
      return insertedRecord;
    }
    throw new Error("Insert operation failed");
  }
  async executeUpdateQuery(env, schema, table, params) {
    const { id, ...updateData } = params;
    if (!id) {
      throw new Error("ID is required for update operation");
    }
    const columns = Object.keys(updateData).filter((key) => updateData[key] !== void 0);
    ...
    if (result.success && result.changes > 0) {
      const updatedRecord = await env.DB.prepare(`SELECT * FROM ${table} WHERE id = ?`).bind(id).first();
      return updatedRecord;
    }
    throw new Error("Update operation failed or no records affected");
  }
  async executeDeleteQuery(env, schema, table, params) {
    const { id } = params;
    if (!id) {
  ...
  mapSQLiteTypeToSQLType(sqliteType) {
  ...
  register(app2) {
    app2.post("/api/auth/login", async (c) => {
      try {
        ...
        const token = await sign2(payload, c.env.JWT_KEY);
        return c.json({
          token,
    ...
    app2.get("/api/auth/validate", async (c) => {
      try {
...
__name(AuthEndpoints, "AuthEndpoints");

// src/endpoints/dynamic-crud.ts
var DynamicCrudEndpoints = class {
  constructor(dbService2) {
    this.dbService = dbService2;
  }
  register(app2) {
    app2.get("/api/tables", async (c) => {
      try {
    ...
    app2.get("/api/tables/:schema/:table/schema", async (c) => {
      try {
    ...
    app2.get("/api/:schema/:table", async (c) => {
      try {
        const schema = c.req.param("schema");
        const table = c.req.param("table");
        const page = parseInt(c.req.query("page") || "1");
        const pageSize = Math.min(parseInt(c.req.query("pageSize") || "100"), 1e3);
    ...
    app2.get("/api/:schema/:table/:id", async (c) => {
      try {
        const schema = c.req.param("schema");
        const table = c.req.param("table");
    ...
    app2.post("/api/:schema/:table", async (c) => {
      try {
        const schema = c.req.param("schema");
        const table = c.req.param("table");
    ...
    app2.put("/api/:schema/:table/:id", async (c) => {
      try {
        const schema = c.req.param("schema");
        const table = c.req.param("table");
        const id = c.req.param("id");
    ...
    app2.delete("/api/:schema/:table/:id", async (c) => {
      try {
        const schema = c.req.param("schema");
        const table = c.req.param("table");
        const id = c.req.param("id");
...
__name(DynamicCrudEndpoints, "DynamicCrudEndpoints");
...
var crudEndpoints = new DynamicCrudEndpoints(dbService);
...
// .wrangler/tmp/bundle-hNa5cW/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-hNa5cW/middleware-loader.entry.ts
